...
def image_to_embedding(image_path: Optional[str] = None, img = None):
    ...
    Args:
        image_path (Optional[str]): Path to imag to be converted to an embedding. If not provided, user must instead directly pass in an image.
...
def generate_input_data(image_directory: str):
    ...
    Args: 
        image_directory (str): Image directory containing frames from a videl.
    Returns:
...
def return_predicted_masked_image(image_paths: List[str], index: int, weights: np.ndarray):
    """
...
def combine_true_bboxes(true_bboxes: List[np.ndarray], og_cols: int, og_rows: int, scale_to_grid: bool = False):
    """
    ...
    for object_bboxes in true_bboxes:
        ...
        for time in range(timesteps):
            ...
            if scale_to_grid:
                true_mask = scaled_mask_from_bbox(true_bbox, og_rows = og_rows, og_cols = og_cols)
            else:
                true_mask = true_mask_from_bbox(true_bbox, og_rows = og_rows, og_cols = og_cols)
            updated_foreground_slice = np.where(true_mask == 1, 1, existing_foreground_slice)
...
def scaled_mask_from_bbox(true_bbox: np.ndarray, og_cols: int, og_rows: int):
    ...
    """
    true_mask = true_mask_from_bbox(true_bbox = true_bbox, og_cols = og_cols, og_rows = og_rows)
    
    #Scan over the image in blocks of 32x32
    scaled_mask = true_mask.copy()
    for x_scan in range(7):
        for y_scan in range(7):
            if np.any(true_mask[x_scan*32: (x_scan + 1)*32, y_scan*32: (y_scan + 1)*32] == 1):
                scaled_mask[x_scan*32: (x_scan + 1)*32, y_scan*32: (y_scan + 1)*32] = 1
    return scaled_mask
        
def evaluate_bbox_prediction(true_bboxes: List[np.ndarray], weights: np.ndarray, og_cols: int,
                             og_rows: int, scale_to_grid = False):
    """
    ...
    true_masks = combine_true_bboxes(true_bboxes, og_cols = og_cols, og_rows = og_rows, scale_to_grid = scale_to_grid)
    ...
    for index in range(timesteps):
        ...
        jaccard_index = compare_true_vs_predicted_mask(true_mask = true_masks[index], predicted_mask = extended_mask)
...
def compare_true_vs_predicted_mask(true_mask: np.ndarray, predicted_mask: np.ndarray):
    ...
    for cluster in np.unique(flat_predicted_bbox):
        ...
        union = np.sum(true_mask == 1) + np.sum(predicted_mask == cluster) - intersection
...
def return_bbox_image(root_filename: str, image_paths: List[str], index: int, annotation_df: pd.DataFrame, 
                      video_df: pd.DataFrame, weights: Optional[np.ndarray] = None, scale_to_grid = False):
    """
    Return a version of an image masked according to its ground truth and optionally also predicted bboxes.
    ...
    Args:
        ...
        weights (np.ndarray): Cluster membership weights from STGkM. If not provided, will not be visualized.
        scale_to_grid (bool). Whether or not to scale the ground truth bboxes to a 7x7 grid.
    """
    img = Image.open(image_paths[index])
    cols, rows = img.size
    
    bboxes = get_true_bboxes(root_filename = root_filename, video_df=video_df, annotation_df= annotation_df)
    true_foreground_masks = combine_true_bboxes(true_bboxes = bboxes, og_cols = cols, og_rows = rows, scale_to_grid=scale_to_grid)
    ...
    plt.imshow(true_foreground_mask, alpha = .3)
    plt.show()
    return None

def get_true_bboxes(root_filename: str, video_df: pd.DataFrame, annotation_df: pd.DataFrame):
    """
