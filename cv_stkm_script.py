...
TRIALS = 5
COUNT = 0
RESULTS_FILE = 'stgkm_computer_vision_df_cosine.pkl'
...
for dir_name in glob.iglob('cv_data/train/JPEGImages/**/', recursive = True):
    if len(dir_name) == 28:
        ...
        if os.path.isdir(dir_name):
            ...
            tracked_objects = get_true_bboxes(root_filename=root_filename, annotation_df=annotation_df, video_df=video_df)
            ...
            for trial in range(TRIALS):
                ...
                new_score = evaluate_bbox_prediction(true_bboxes = tracked_objects,
                                                             weights = tkm.weights,
                                                             og_cols = og_size[0],
                                                             og_rows = og_size[1],
                                                             scale_to_grid = True)
