...
class TKM:
    ...
    def perform_clustering_avg_centers(
        ...
        weights = np.transpose(np.dstack([np.random.rand(n,k)]*t), axes = [2,0,1])
        centers_average = np.cumsum(centers, axis = 0)/np.arange(1,t+1)[:, np.newaxis, np.newaxis]

        while err >= tol:

            data_norm = np.linalg.norm(self.data, 2, axis=1) ** 2

            centers_new = (self.data @ weights + n * lam * centers_average) / (
                np.sum(weights, axis=1)[:, np.newaxis, :] + lam * n
            )
            ...
            sum_term_2 = np.sum(
                n * lam * np.linalg.norm(centers_new - centers_average, 2, axis=1) ** 2
            )
